...
srv.on('connection', function(socket){
  ...
  */
  ...
  socket.on('measure', function (p) {
    console.log(p);
    exec("python a.py", (err, stdout, stderr)=>{
      console.log(stdout);
    });
  });
/*
        ...
        for (let sss of soushin){
          if(sss.Script !== 'NoScript'){
            ...
            */
            //windows用
            sss.HelpUsage = resExec.stdout.split("optional arguments:")[0].match(/(?<=\[).*?(?=\])/g);
            sss.HelpArgument = resExec.stdout.split("optional arguments:")[1].match(/(?<=\s{1}[-]{1}.{9}\s{2}).+(?=\s)/g);
            
            /*ノートパソコン用tama
            sss.HelpUsage = resExec.stdout.split("options:")[0].match(/(?<=\[).*?(?=\])/g);
            sss.HelpArgument = resExec.stdout.split("options:")[1].match(/(?<=\s{1}[-]{1}.{9}\s{2}).+(?=\s)/g);
            */
