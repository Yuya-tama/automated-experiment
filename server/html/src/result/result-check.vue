...
<div class="uk-card uk-card-body uk-card-default">
    <h3 class="uk-card-title">{{$store.state.resultRun.result_title}}</h3>
    <div class="uk-card uk-card-body uk-card-default">
        <h4>Date</h4>
        <div>{{$store.state.resultRun.created_at}}</div>
        <h4>Comment</h4>
        <div v-show="$store.state.resultRun.comment !== null">
            {{$store.state.resultRun.comment}}
        </div>
        <div v-show="$store.state.resultRun.comment == null">
            No comment
        </div>
        <a href='#edit-comment' uk-toggle @click='resultCommentSet'>Edit comment</a>
    </div>
    <!-- 以下モーダルウインドウ -->
    <div id='edit-comment' uk-modal>
    <div class='uk-modal-dialog uk-modal-body'>
        <h2 class='uk-modal-title'>Edit comment</h2>
        <textarea v-model.trim='resultRunComment' type='text' style="width:400px; height:200px" class='uk-textarea'></textarea><br>
        <p class='uk-text-right'>
        <button class="uk-button uk-button-default uk-modal-close" type="button">Cancel</button>
        <button class="uk-button uk-button-primary uk-modal-close" @click='resultCommentSubmit' type="button">OK</button> 
      </p>
    </div>
    </div>
    <div class="uk-card uk-card-body uk-card-default">
            ...
            <li>
                ...
                <div>
                    <table class='uk-table uk-table-divider'>
                        ...
                        <tbody>
                        <tr v-for='(elem,index) in $store.state.resultDevice' :key="elem.id">
                            <td>{{index + 1}}</td>
            ...
            <li>
                ...
                <div>
                    <table class='uk-table uk-table-divider'>
                        ...
                        <tbody>
                        <tr v-for='elem in $store.state.resultData' :key="elem.id">
            ...
            <li>
                ...
                <div>
                    <table class='uk-table uk-table-divider'>
                        ...
                        <tbody>
                        <tr v-for='elem in $store.state.resultTimeLogData' :key="elem.id">
                            <td>{{elem.device_name }}</td>
                            <td>{{elem.created_at}}</td>
                            <td>{{elem.value}}</td>
                        </tr>
...
export default {
    ...
    methods:{
        ...
        resetOutline(){
            ...
            this.displayProcedure = JSON.parse(JSON.stringify(this.$store.state.resultProcedure));
        },
        ...
        selectedOutline(index){
            ...
            this.tempOutline = Object.assign({}, this.tempOutline, this.$store.state.resultProcedure[index]);
        },

        displayDevice(deviceId){
            const dev = this.$store.state.resultDevice.find(elem => elem.id == deviceId);
        ...
        deviceIdExtractForLog(){
            let array = this.$store.state.resultTimeLogData.map(function(val){
        ...
        chartDataSelect(deviceId){
            let val = this.$store.state.resultTimeLogData.filter(a => a.result_device_id  == deviceId);
        ...
        deviceNameFind(deviceId){
            let a = this.$store.state.resultTimeLogData.find(elem => elem.result_device_id == deviceId)

            return a.device_name
        },

        resultCommentSubmit(){
            if (this.resultRunComment == ''){
                this.resultRunComment = null;
            }
            this.$emit('resultCommentSubmit',this.resultRunComment,this.resultRunId);
         },

        resultCommentSet(){
             this.resultRunComment=this.$store.state.resultRun.comment;
         },
    },
...
/*
    mounted(){
        this.$store.subscribe((mutation, state) => {
            if (mutation.type === 'resultProcedure'){
                this.resetOutline();
            }
        })
    },
      ...
      'resultProcedure',
        ...
        displayBlock: function(){
            if(this.isAllOutlineSelected){
                let b = this.$store.state.resultBlock
                return b
            }

            let ii = this.tempOutline.id;
            let block = this.$store.state.resultBlock.filter(function(elem){
