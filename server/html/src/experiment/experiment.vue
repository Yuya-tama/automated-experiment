...
<div class="uk-card uk-card-body uk-card-default">
    <h4 v-show='!$store.state.experimentTitle.isDoing'>Experiment is not running</h4>
    <div v-show='$store.state.experimentTitle.isDoing'>
    <ul uk-accordion>
      <li class='uk-open'>
        <a class='uk-accordion-title' href='#'>{{$store.state.experimentTitle.experiment_title}}</a>
        <div class='uk-accordion-content'>
          <p  class='uk-width-2-3'> 
            <span class='run-infomation'>RunNumber</span>: {{$store.state.runInfo.id}}<br>
            <span class='run-infomation'> Date</span>: {{$store.state.runInfo.created_at}}<br>
            <span class='run-infomation'> Comment</span>: {{$store.state.runInfo.comment}}<br>
            <a href='#new-comment' uk-toggle @click='commentSet'>Add comment</a>
          </p>      
        </div>
      </li>
    </ul>
     <div v-show='$store.state.experimentTitle.isDone'><span uk-icon="icon: check; ratio:1.2"></span>Done</div>
     <button class="uk-button uk-button-danger" uk-toggle='target: #experiment-quit'>Finish this experiment</button>
     <div v-show='!$store.state.experimentTitle.isDone && !isBreak'><span uk-spinner='ratio:0.7'></span>Running</div>
     <div v-show='isBreak'><span uk-icon='icon: minus-circle'></span>Break</div>
     <div v-show='$store.state.experimentTitle.statusMsg' class="uk-text-center box1">
        <h2><font color="red">!! Notice !!</font></h2>
        <p class="uk-text-large"><font color="red">
        {{$store.state.experimentTitle.statusMsg}}
        </font></p>
      </div>
     <p></p>
    <div v-if='!isBreak && !$store.state.experimentTitle.isDone'>
    ...
    <div v-if='$store.state.experimentTitle.isDone'>
      <button class="uk-button uk-button-danger" disabled>Break</button>
      <button class="uk-button uk-button-default"  disabled>Resume</button>
    </div>
    ...
    <br>
     ...
     <div>
      Time elapsed (this procedure) : {{$store.state.sstateNow.timeDiff}}<span v-if="$store.state.sstateNow.timeDiff == null">- </span> second
      <div v-if="$store.state.sstateNow.temp !== null">Temperature : {{$store.state.sstateNow.temp}} ℃</div>
     </div>
      ...
      <ul class="uk-switcher uk-margin" id="experiment-process-contents">
        <li>
          <table class="uk-table uk-table-hover uk-width-2-3">
            ...
            <tbody>
              <tr v-for="(b,index) in $store.state.experimentRecipe" :key='b.procedureOrder'>
                <td>{{index+1}}</td>
...
export default {
  ...
  methods :{
    ...
    commentSubmit(){
      this.$emit('commentSubmit',this.runComment,this.$store.state.runInfo.id);
    },

    commentSet(){
      this.runComment=this.$store.state.runInfo.comment;
    },
