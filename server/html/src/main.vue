...
-->
<div>
    <!-- <button class="uk-button uk-button-primary uk-width-1-5" type="button" @click='check()'>check</button> -->
</div>
<div v-show="!isEdit && !isResult">
  ...
  <ul class="uk-switcher uk-margin" id="subnav-main-contents">
    <li>
      <flexible-recipe-list  @createRecipe="createRecipe" 
      
      @experimentStart='experimentStart' @recipeEdit='recipeEdit' @recipeTrash="recipeTrash"
      @actionEmit='actionEmit' @editProcedure='editProcedure'
      @selectEditTitle="selectEditTitle"
      ref="flRecipeList"></flexible-recipe-list>
    </li>
    <li>
      <client-list  @clientAction="clientAction"></client-list>
    </li>
    <li>
      <experiment ref='experimentChild'
      @procedureBreak='procedureBreak' @commentSubmit='commentSubmit'
    ...
    <li><setting @editSetting="editSetting"></setting></li>
...
<div v-show="isResult">
  ...
  <ul class="uk-switcher uk-margin" id="result-check-contents">
    ...
    @resultCommentSubmit='resultCommentSubmit'
...
/*
      this.$socket.on('recipeList',(a) => {
      this.$store.commit("recipeList",a);
    ...
    this.$socket.on('experimentTitle',(title) =>{
      this.$store.state.experimentTitle = title;
    });

    this.$socket.on('experimentRecipe',(procedure) =>{
      this.$store.state.experimentRecipe = procedure;
    });
...
*/
    this.$socket.on('valueChoiceList',(a)=>{
      this.$store.commit('valueChoiceList',a);
    });
...
/*
    ...
    this.socket.on('resultCommentGet',(a)=>{
      this.resultRun = a;
    });
*/
    this.$socket.on('experimentDoneReturn',()=>{
      //実験が終わったので各々の情報をリセット
      Object.keys(this.$store.state.experimentTitle).forEach(key => {
        this.$delete(this.$store.state.experimentTitle,key);
      });
      Object.keys(this.$store.state.runInfo).forEach(key => {
        this.$delete(this.$store.state.runInfo,key);
      });
      this.$store.state.experimentRecipe.splice(0,this.$store.state.experimentRecipe.length);
    });
/*
...
*/    
  ...
  methods :{
    ...
    resultCommentSubmit(comment,runNumber){
       if(this.isConnect === true){
        this.$socket.emit('resultCommentSubmit' ,comment,runNumber);
      }else{
        alert('実行に失敗しました\nサーバーとの接続を確認してください');
      };
    },
    ...
    check(){
        console.table(this.$store.state.resultList);
    }
  },
  ...
  computed:{
    ...
    editBlock:function(){
      ...
      let condition = JSON.parse(JSON.stringify(this.$store.state.conditionList));

      //detailとcondition_idの正しい方を表示させるために(detailView追加)
      for(let i = 0; i < blockListTemp.length; i++){
          if(blockListTemp[i].condition_id){
            let index = this.$store.state.conditionList.findIndex(({id}) => id == blockListTemp[i].condition_id);
    ...
    editDevice:function(){
      let device = this.$store.state.recipeDevice.filter(x => x.experiment_title_id == this.editTitle.id);
      return device
    },
