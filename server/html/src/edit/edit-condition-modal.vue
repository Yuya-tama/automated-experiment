...
export default {
    ...
    computed:{
        action: function(){
            ...
            const action = this.$store.state.actionLinkList.find(elem => elem.id == deviceLinkId);
            if(action == void(0)){
                return {}
            }
        ...
        choices: function(){
            const deviceLinkId = this.editedCondition.device_link_id;
            const choice = this.$store.state.valueChoiceList.filter(elem => elem.device_link_id == deviceLinkId);
