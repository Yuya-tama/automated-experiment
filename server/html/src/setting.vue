...
<div class='uk-card uk-card-body uk-card-default'>
    <div>
        <table class="uk-table uk-table-hover" v-if="!isSetting">
            ...
            <tbody>
            <tr v-for="b in $store.state.settingList" :key='b.id'>
                <td>{{b.explanation}}</td>
                <td>{{b.value}}</td>
                <td>{{b.updated_at}}</td>
            </tr>
...
export default{
    ...
    methods :{
        settingCopy(){
            this.settingListCopy = this.$store.state.settingList.map(x => ({...x}));
        ...
        defaultSettingRevert(){
            let value = this.$store.state.settingDefaultList.map(x => parseFloat(x.value));
