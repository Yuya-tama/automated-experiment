...
srvUser.on("connection", (socket) => {
  ...
  */
  ...
  socket.on("resultCommentSubmit", function (comment, runNumber) {
    con.query(
      "UPDATE runs SET comment= ? WHERE id= ?",
      [comment, runNumber],
      (err, res, fields) => {
        con.query(
          ...
          runNumber,
          (err, res, fields) => {
            socket.emit("resultCommentGet", res[0]);
          }
        );
      }
    );
                    ...
                    con.query(
                      ...
                      (er, re, fi) => {
                        ...
                        if (i == outLength) {
                          ...
                          //resultdata試し
                            con.query(
                              "INSERT INTO result_data (run_id, data, created_at) VALUES (?)",
                              [
                                [runInfo.id,"null",date,],
                              ],
                            );
                          }
                        }  
  ...
  socket.on('getResultForOpt',()=> {
    client.emit('getResultForOpt');
  });

  socket.on('sendData', (data) => {
    console.log(data.variables);
    console.log(data.results);
    client.emit('sendDataToClient',data);
  });
...
client.on("connection", (socket) => {
  ...
  socket.on("ResultForOptimization", (d) =>{
    const data = d["data_temp"];
    const arrayOfKeyValuePairs = [];

    // 各オブジェクトに対して処理を行い、keyValuePairs を作成
    for (const obj of data) {
      ...
      arrayOfKeyValuePairs.push(keyValuePairs);
    }
    ...
    // 配列を文字列に結合
    const joinedString = d.join('');
    console.log(joinedString);

    // 正規表現を使用して文字列（キー）だけを抽出
    const regex = /'([^']+)':/g;
    let match;
    const keys = [];
    ...
    while ((match2 = regex2.exec(joinedString)) !== null) {
      const number = parseFloat(match2[1]);
      numbers.push(number);
    }
    //console.log(numbers);

    
    // keys と numbers を結びつけてオブジェクトの配列を作成
    const keyValuePairs = [];

    for (let i = 0; i < Math.min(keys.length, numbers.length); i++) {
      const key = keys[i];
...
*/
    srvUser.emit("ResultForOptimization",arrayOfKeyValuePairs);
  });

  socket.on("sample", (d) =>{
    srvUser.emit("sample",d);
  });

  socket.on("sendOptimization", (d) =>{
    srvUser.emit("sendOptimization",d);
  })
